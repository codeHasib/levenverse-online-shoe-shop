...
import { ShoppingBag, ChevronRight, Star, ShieldCheck, Truck, Minus, Plus } from "lucide-react";
...
export default function ProductPage() {
  ...
  if (loading) return (
    <div className="h-screen flex items-center justify-center bg-white">
      <div className="text-[10px] tracking-[0.5em] uppercase animate-pulse">Loading Gallery</div>
    </div>
  );

  if (!product) return <div className="p-20 text-center uppercase tracking-widest text-[10px]">Product Unavailable</div>;
        ...
        <div className="grid lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-5 pt-4">
            <nav className="flex items-center gap-2 mb-6 text-[9px] tracking-widest uppercase text-neutral-400">
              <span>Catalog</span> <ChevronRight size={8} /> <span>{product.categoryId?.name}</span>
            </nav>

            <h1 className="text-3xl tracking-[0.2em] uppercase font-light text-black mb-2">{product.title}</h1>
            <p className="text-xl tracking-tighter text-[#0070f3] mb-8 font-normal">৳{product.price}</p>

            <div className="h-[1px] w-full bg-neutral-100 mb-8" />

            <p className="text-[12px] leading-relaxed text-neutral-500 tracking-wide mb-10">
              {product.description}
            </p>
            ...
            <div className="space-y-8">
              {product.sizes?.length > 0 && (
                <div>
                  <span className="text-[9px] tracking-[0.3em] uppercase text-black block mb-4 font-bold">Available Sizes</span>
              ...
              <div className="flex gap-4">
                <div className="flex items-center border border-neutral-200">
                  <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="p-4"><Minus size={12}/></button>
                  <span className="w-8 text-center text-[12px]">{quantity}</span>
                  <button onClick={() => setQuantity(quantity + 1)} className="p-4"><Plus size={12}/></button>
                </div>
            ...
            <div className="mt-12 space-y-4 pt-8 border-t border-neutral-100">
               <div className="flex items-center gap-4 text-[9px] tracking-[0.2em] uppercase text-neutral-500">
                  <Truck size={14} className="text-[#0070f3]" /> <span>Express Shipping (2-3 Days)</span>
               </div>
               <div className="flex items-center gap-4 text-[9px] tracking-[0.2em] uppercase text-neutral-500">
                  <ShieldCheck size={14} className="text-[#0070f3]" /> <span>Secure Payment Guarantee</span>
               </div>
            </div>
          </div>
        </div>
